...
import 'package:Easy_shopping_admin/PdfBook/AddPdfBook.dart';
import 'package:Easy_shopping_admin/PdfBook/EditPdfBook.dart';
import 'package:Easy_shopping_admin/PdfViewScreen.dart';
...
class PdfBookScreen extends StatefulWidget {
  @override
  _PdfBookScreenState createState() => _PdfBookScreenState();
}
...
class _PdfBookScreenState extends State<PdfBookScreen> {
  ...
  Future<void> _showCategory() async {
    var res = await CallApi().getData('/api/showAllPdf');
    ...
    if (res.statusCode == 200) {
      ...
      store.dispatch(PdfBookListAction(body['pdfBooks']));
    ...
    store.dispatch(PdfBookLoadingAction(false));
  }
              ...
              return Scaffold(
                  ...
                  body:RefreshIndicator(
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: store.state.pdfBookLoading
                          ? Center(
                            child: Container(
                               margin: EdgeInsets.only(top:MediaQuery.of(context).size.height/3),
                                                      child: CircularProgressIndicator(),
                          ))
                      : store.state.pdfBookList.length==0
                          ? Center(
                            child: Container(
                               margin: EdgeInsets.only(top:MediaQuery.of(context).size.height/3),
                                child: Text(
                  ...
                  floatingActionButton: FloatingActionButton(
                    onPressed: () {
                     Navigator.push(context, FadeRoute(page: AddPdfBook()));
                    },
                    backgroundColor: appColor,
  ...
  List<Widget> _showCategoryList() {
    ...
    for (var d in store.state.pdfBookList) {
      ...
      list.add(
        GestureDetector(
          onTap: (){
             Navigator.push(context, FadeRoute(page: PdfViewScreen(d)));
            
          },
          child: categoryCard(d)));
    }
                  ...
                  children: <Widget>[
                    ListTile(
                        ...
                        onTap: () {
                          ...
                          Navigator.push(context, SlideLeftRoute(page: EditPdfBook(d)));
  ...
  void _delete(d) async {
    ...
    var res = await CallApi().postData(data, '/api/deletePdfBook');
    ...
    print(body);
      ...
      if (res.statusCode == 200) {
        ...
        for(var data in store.state.pdfBookList){
          if(data['id']==d['id']){

            store.state.pdfBookList.remove(d);
            break;
