...
import 'package:Easy_shopping_admin/ProductsCard/EmptyProductCard.dart';
...
class EmptyStockProduct extends StatefulWidget {
  @override
  _EmptyStockProductState createState() => _EmptyStockProductState();
}

class _EmptyStockProductState extends State<EmptyStockProduct> {
  ...
  void initState() {
    ...
    var res = await CallApi().getData('/api/showStockOutProduct');
    ...
    if (res.statusCode == 200) {
      ...
      store.dispatch(EmptyStockProductListAction(body['Product']));
    ...
    store.dispatch(EmptyStockProductLoadingAction(false));
  }
  ...
  Widget build(BuildContext context) {
    return StoreProvider<AppState>(
        ...
        body: SafeArea(
          child: store.state.emptyStockProductLoading
            ? Center(
                child: CircularProgressIndicator(),
              )
            : store.state.emptyStockProductList.length == 0
                ? RefreshIndicator(
                            ...
                            children: <Widget>[
                              Container(
                                ...
                                child: GridView.builder(
                                            ...
                                            itemCount: store.state.emptyStockProductList.length,
                                          )
